		...
		<Link href={href} prefetch={false}>
			<a>
				<ListItem button key={text}>
					<ListItemIcon>
						{number === 0 ? <ExploreIcon /> : number === 1 ? <PersonOutlineIcon /> : <CreateIcon />}
					</ListItemIcon>
...
const Menu: React.FC<Props> = ({ handleOpen, open }) => {
	console.log(handleOpen, open)
	const list = () => (
		<Box sx={{ width: 250 }} role="presentation" onClick={handleOpen} onKeyDown={handleOpen}>
			<List>
